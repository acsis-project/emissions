...
;
;
;   combine_all_sources_NH3_1960-2020.pro
;
;
;   This program reads pre-processed emissions files for ammonia from
;
;    * anthropogenic MACCity emissions
;    * biomass burning MACCity emissions
;    * oceanic emissions from Bouwman et al. 1997
;
...
anthrop_file = ukca_gws+'emissions/ACCMIP-MACCity_anthrop_1960-2020/NH3/newfile.nc'
bioburn_file = ukca_gws+'emissions/ACCMIP-MACCity_bioburn_1960-2008/NH3/historic/newfile.nc'
oceanic_file = ukca_gws+'emissions/other/ocn-nh3.cej'

; output file:

ofn          = ukca_gws+'emissions/combined_1960-2020/combined_sources_NH3_1960-2020_360d.nc'
;ofn          = ukca_gws+'emissions/combined_1960-2020/combined_sources_NH3_1960-2020_greg.nc'
...
; the latitudes in the anthropogenic MACCity emissions are N-->S which is different
; from the biogenic MACCity emissions.
...
for k=0,n_elements(lats)-1 do begin
   ...
   tmpfield[*,k_bar,t] = anthrop_flux[*,k,t]
endfor
endfor

anthrop_flux = tmpfield
...
; read raw data file at 1x1 degree resolution

ocean_nh3 = fltarr(360, 180)

; raw data is 1x1 degree, ordered S --> N, -180 --> 180

openr, nh3unit, oceanic_file, /get_lun

line=''
readf, nh3unit, line ; skip header line

while (not eof(nh3unit)) do begin
  readf, nh3unit, lon_nh3, lat_nh3, nh3
  ilon = fix(lon_nh3) + 180
  ilat = fix(lat_nh3) +  90
  ocean_nh3[ilon, ilat] = nh3 ; kg[N]/gridbox/year
endwhile
free_lun, nh3unit

; read 1x1 degree gridbox surface area

surf_1x1_file = ukca_gws + 'data/surf_one_by_one_2.nc'

ncid = ncdf_open(surf_1x1_file,/nowrite)
varid  = ncdf_varid(ncid,'surf')
ncdf_varget,ncid,varid,surf_1x1
ncdf_close,ncid

; adjust units to kg[NH3]/m2/s

NtoNH3=double(17.031/14.007)

ocean_nh3[*,*] = ocean_nh3[*,*] / surf_1x1[*,*] / (3600.0*24.0*365.0) * NtoNH3 ; kg[NH3]/m2/s

; re-grid data to 0.5 x 0.5 degree grid

print,'regridding oceanic emissions'

; As we are dealing with fluxes (kg m-2 s-1) the values of the input field will be simply
; assigned to output grid without change of value.

oceanic_flux = fltarr(720,360)   ; Output field with 0.5 x 0.5 degrees (lon x lat)
                                    ; and 12 monthly time steps
...
for k=0,179 do begin
  ...
  for i=0,359 do begin
    ...
    oceanic_flux[ii  ,kk  ] = ocean_nh3[i,k]
    oceanic_flux[ii+1,kk  ] = ocean_nh3[i,k]
    oceanic_flux[ii  ,kk+1] = ocean_nh3[i,k]
    oceanic_flux[ii+1,kk+1] = ocean_nh3[i,k]
...
tmpfield = dblarr(n_elements(lons),n_elements(lats))

for i = 0,(n_elements(lons)/2)-1 do $
   tmpfield[i,*] = oceanic_flux[i+(n_elements(lons)/2),*]

for i = (n_elements(lons)/2),n_elements(lons)-1 do $
   tmpfield[i,*] = oceanic_flux[i-(n_elements(lons)/2),*]

oceanic_flux = tmpfield
...
allflux = anthrop_flux + bioburn_flux
...
for t=0,n_elements(times)-1 do begin
  ...
  allflux[*,*,t] = allflux[*,*,t] + oceanic_flux[*,*]
...
openw, unit1, ukca_gws+'emissions/combined_1960-2020/NH3_monthly_combined.csv', /get_lun
openw, unit2, ukca_gws+'emissions/combined_1960-2020/NH3_annual_combined.csv', /get_lun
...
ncdf_attput,ncid,fieldvar_id,'long_name','Surface NH3 emissions'
ncdf_attput,ncid,fieldvar_id,'molecular_weight',17.031,/float
ncdf_attput,ncid,fieldvar_id,'molecular_weight_units','g mol-1'

ncdf_attput,ncid,/global,'history','combine_all_sources_NH3_1960-2020.pro'
ncdf_attput,ncid,/global,'file_creation_date',systime(/utc)+' UTC'
ncdf_attput,ncid,/global,'description','Time-varying monthly surface emissions of ammonia (NH3) from 1960 to 2020.'
ncdf_attput,ncid,/global,'source','The emissions flux in this file comprises combined emissions from anthropogenic, biomass burning, and oceanic sources. MACCity provides anthropogenic emissions from 1960 to 2020 and biomass burning emissions from 1960 to 2008. Biomass burning emissions from 2009 to 2020 have been taken from the ACCMIP linearly interpolated RCP8.5 data set. Oceanic emissions are taken from Bouwman et al. 1997 and applied perpetually for all years.'
...
ncdf_attput,ncid,/global,'global_total_emissions_2000','53.793 Tg NH3 per year'
