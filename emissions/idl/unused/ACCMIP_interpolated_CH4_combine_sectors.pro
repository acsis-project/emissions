...
openw, unit1, workspace+'ACCMIP_annual_CH4.csv', /get_lun
openw, unit2, workspace+'ACCMIP_anthrop_monthly_CH4.csv', /get_lun
...
for yr=0,n_elements(year)-1 do begin
  ...
  ems_file1  = 'accmip_interpolated_emissions_'+scenario+'_CH4_anthropogenic_'+syear+'_0.5x0.5.nc'
  ems_file2  = 'accmip_interpolated_emissions_'+scenario+'_CH4_biomassburning_'+syear+'_0.5x0.5.nc'
  ems_file3  = 'accmip_interpolated_emissions_'+scenario+'_CH4_ships_'+syear+'_0.5x0.5.nc'
  ems_ofn    = 'accmip_interpolated_emissions_'+scenario+'_CH4_all_sectors_'+syear+'_0.5x0.5.nc'
  ...
  ; add all anthropogenic sectors (for CH4 that's 0-6 and 9)
  for t = 0,n_elements(times)-1 do begin
     monthtotal = total(emsfield[*,*,t,0]*surf[*,*]*double(numdays[t])*secs_per_day,/double) $
                ...
                + total(emsfield[*,*,t,9]*surf[*,*]*double(numdays[t])*secs_per_day,/double)
     printf, unit2, syear+'-'+strcompress(string(t+1),/remove_all)+' , ',monthtotal
  endfor

  ; write out individual sector contributions to csv file
  printf, unit1, syear,',',emstotal[ 0],',',$
                           ...
                           emstotal[ 7],',',$
                           emstotal[ 8],',',$
                           emstotal[ 9],',',total(emstotal,/double)
