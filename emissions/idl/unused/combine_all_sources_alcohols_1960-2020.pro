...
;
;
;   combine_all_sources_CH3OH_1960-2020.pro
;
;
;   This program reads pre-processed emissions files for methanol from
;
;    * anthropogenic MACCity emissions
...
anthrop_file1 = ukca_gws+'emissions/ACCMIP-MACCity_anthrop_1960-2020/CH3OH/newfile.nc'
anthrop_file2 = ukca_gws+'emissions/ACCMIP-MACCity_anthrop_1960-2020/other_alcohols/newfile.nc'
bioburn_file  = ukca_gws+'emissions/ACCMIP-MACCity_bioburn_1960-2008/alcohols/historic/newfile.nc'
biogen_file1  = ukca_gws+'emissions/MEGAN-MACC_1980-2010/MEGAN-MACC_biogenic_CH3OH_processed.nc'
biogen_file2  = ukca_gws+'emissions/MEGAN-MACC_1980-2010/MEGAN-MACC_biogenic_ethanol_processed.nc'

; output file:

ofn          = ukca_gws+'emissions/combined_1960-2020/combined_sources_alcohols_1960-2020_360d.nc'
;ofn          = ukca_gws+'emissions/combined_1960-2020/combined_sources_alcohols_1960-2020_greg.nc'
gregorian    = 0  ; set to 1 for Gregorian or 0 for 360-day calendar
...
print,'reading first anthropogenic emissions fluxes'

ncid = ncdf_open(anthrop_file1,/nowrite)
timeid = ncdf_varid(ncid,'time')
ncdf_varget,ncid,timeid,times
varid  = ncdf_varid(ncid,'emiss_flux')
ncdf_varget,ncid,varid,anthrop_flux1
ncdf_close,ncid
...
for k=0,n_elements(lats)-1 do begin
   ...
   tmpfield[*,k_bar,t] = anthrop_flux1[*,k,t]
endfor
endfor

anthrop_flux1 = tmpfield
...
print,'reading second anthropogenic emissions fluxes'

ncid = ncdf_open(anthrop_file2,/nowrite)
timeid = ncdf_varid(ncid,'time')
ncdf_varget,ncid,timeid,times
varid  = ncdf_varid(ncid,'emiss_flux')
ncdf_varget,ncid,varid,anthrop_flux2
ncdf_close,ncid
...
for k=0,n_elements(lats)-1 do begin
   k_bar = (n_elements(lats)-1)-k   ; k counting up from 0, k_bar counting down from 359
   tmpfield[*,k_bar,t] = anthrop_flux2[*,k,t]
endfor
endfor

anthrop_flux2 = tmpfield
...
print,'reading first biogenic emissions fluxes'

ncid = ncdf_open(biogen_file1,/nowrite)
varid  = ncdf_varid(ncid,'emiss_flux')
ncdf_varget,ncid,varid,biogen_flux1
ncdf_close,ncid
...
for i = 0,(n_elements(lons)/2)-1 do $
   tmpfield[i,*,*] = biogen_flux1[i+(n_elements(lons)/2),*,*]

for i = (n_elements(lons)/2),n_elements(lons)-1 do $
   tmpfield[i,*,*] = biogen_flux1[i-(n_elements(lons)/2),*,*]

biogen_flux1 = tmpfield
...
print,'reading second biogenic emissions fluxes'

ncid = ncdf_open(biogen_file2,/nowrite)
varid  = ncdf_varid(ncid,'emiss_flux')
ncdf_varget,ncid,varid,biogen_flux2
ncdf_close,ncid
...
for i = 0,(n_elements(lons)/2)-1 do $
   tmpfield[i,*,*] = biogen_flux2[i+(n_elements(lons)/2),*,*]
...
;---- convert all fluxes to Carbon

print,'converting fluxes to carbon...'

molw_carbon    = double(12.011)
molw_anthrop1  = double(32.04)
molw_anthrop2  = double(32.04)
molw_bioburn   = double(37.63)  ; given in README.accmip_maccity_biomassburning.txt
molw_biogenic1 = double(32.04)
molw_biogenic2 = double(46.07)

anthrop_flux1 = anthrop_flux1 * (molw_carbon/molw_anthrop1)
anthrop_flux2 = anthrop_flux2 * (molw_carbon/molw_anthrop2)
bioburn_flux  = bioburn_flux * (molw_carbon/molw_bioburn)
biogen_flux1 = biogen_flux1 * (molw_carbon/molw_biogenic1)
biogen_flux2 = biogen_flux2 * (molw_carbon/molw_biogenic2)
...
allflux = anthrop_flux1 + anthrop_flux2 + bioburn_flux + biogen_flux1 + biogen_flux2
...
openw, unit1, ukca_gws+'emissions/combined_1960-2020/Alcohols_monthly_combined.csv', /get_lun
openw, unit2, ukca_gws+'emissions/combined_1960-2020/Alcohols_annual_combined.csv', /get_lun
...
ncdf_attput,ncid,fieldvar_id,'long_name','Surface alcohol emissions expressed as carbon'
...
ncdf_attput,ncid,/global,'history','combine_all_sources_alcohols_1960-2020.pro'
ncdf_attput,ncid,/global,'file_creation_date',systime(/utc)+' UTC'
ncdf_attput,ncid,/global,'description','Time-varying monthly surface emissions of lumped alcohols from 1960 to 2020, expressed as carbon.'
...
ncdf_attput,ncid,/global,'global_total_emissions_2000','58.210 Tg C per year'
