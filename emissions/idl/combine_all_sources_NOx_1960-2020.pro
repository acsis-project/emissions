...
;
;
;   combine_all_sources_NOx_1960-2020.pro
;
;
;   This program reads pre-processed emissions files for oxides of nitrogen from
;
;    * anthropogenic MACCity emissions
;    * biomass burning MACCity emissions
;    * soil emissions (Yienger & Levy 1995) annual cycle applied perpetually 
;      over all years
;
...
anthrop_file = ukca_gws+'emissions/ACCMIP-MACCity_anthrop_1960-2020/NOx/newfile.nc'
...
ofn          = ukca_gws+'emissions/combined_1960-2020/combined_sources_NOx_1960-2020_360d.nc'
;ofn          = ukca_gws+'emissions/combined_1960-2020/combined_sources_NOx_1960-2020_greg.nc'
...
print,'reading soil emissions fluxes'

ncid = ncdf_open(biogen_file,/nowrite)
varid  = ncdf_varid(ncid,'NOx')
ncdf_varget,ncid,varid,biogen_flux
ncdf_close,ncid

print,'scaling soil emissions to 12.0 Tg NO/yr'

totsoilnox = double(12.e+09)  ; in kg

; first get surface area
ncid = ncdf_open(surf_file,/nowrite)
varid  = ncdf_varid(ncid,'surf')
ncdf_varget,ncid,varid,surf
ncdf_close,ncid

; now sum up total NOx soil emissions per year
totalyear=0.d
for mth=0,11 do begin
    totalmonth = total(biogen_flux[*,*,mth]*surf[*,*]*numdays[mth]*secs_per_day,/double)
    totalyear = totalyear + totalmonth
endfor

scale = totsoilnox / totalyear
biogen_flux = biogen_flux * scale
...
; add 12 monthly soil flux perpetually for each year:
...
for year=0,numyears-1 do begin
  for mth=0,n_elements(numdays)-1 do begin
     ...
     time = time+1
     allflux[*,*,time-1] = allflux[*,*,time-1] + biogen_flux[*,*,mth]
...
openw, unit1, ukca_gws+'emissions/combined_1960-2020/NOx_monthly_combined.csv', /get_lun
openw, unit2, ukca_gws+'emissions/combined_1960-2020/NOx_annual_combined.csv', /get_lun
...
ncdf_attput,ncid,fieldvar_id,'long_name','Surface NOx emissions expressed as NO'
ncdf_attput,ncid,fieldvar_id,'molecular_weight',30.01,/float
ncdf_attput,ncid,fieldvar_id,'molecular_weight_units','g mol-1'

ncdf_attput,ncid,/global,'history','combine_all_sources_NOx_1960-2020.pro'
...
ncdf_attput,ncid,/global,'global_total_emissions_2000','90.740 Tg NO per year'
