...
;
;
;   MEGAN-MACC_biogenic_other_ketones_preprocess.pro
;
;
;   This program reads multi-annual MEGAN-MACC biogenic emissions files
;   (with monthly emissions fluxes) and produces an extended time series 
;   emissions data file.
;
;
...
ifn = 'MEGAN-MACC_biogenic_other_ketones_1980-2010_4510.nc'

ofn = 'MEGAN-MACC_biogenic_other_ketones_processed.nc'
surffile = '/home/users/mkoehler/ukca_gws/data/surf_half_by_half_2.nc'

molw_Me2CO=58.08d
molw_oKet=72.d  ; from input file meta data

scale = molw_Me2CO/molw_oKet

numyears = 31   ; 1980 -> 2010
totalyears = 61 ; 1960 -> 2020
...
;---- scale emissions flux to target species

biogflux = biogflux * scale
...
anninc=[0,12,24,36,48]  ; timestep increment for the same month of the first five years

; annual emissions flux file for years < 1980
pre1980fld   = fltarr(n_elements(lons),n_elements(lats),12)

; annual emissions flux file for years > 2010
post2010fld  = fltarr(n_elements(lons),n_elements(lats),12)


for mth=0,11 do begin

  ; calculate average over the first five years 1980-1984

  for yr=0,n_elements(anninc)-1 do begin
     pre1980fld[*,*,mth] = pre1980fld[*,*,mth] + biogflux[*,*,mth+anninc[yr]]
  endfor
  pre1980fld[*,*,mth] = pre1980fld[*,*,mth] / n_elements(anninc)

  ; calculate average over the last five years 2006-2010

  for yr=0,n_elements(anninc)-1 do begin
     idx = mth+(((numyears*12)-11)-anninc[yr])
     post2010fld[*,*,mth] = post2010fld[*,*,mth] + biogflux[*,*,idx-1]
  endfor
  post2010fld[*,*,mth] = post2010fld[*,*,mth] / n_elements(anninc)
...
openw, unit1, workspace+'/MEGAN-MACC_biogenic_other_ketones_monthly.csv', /get_lun
openw, unit2, workspace+'/MEGAN-MACC_biogenic_other_ketones_annual.csv', /get_lun


;---- calculate annual totals for entire time series
...
outfield   = fltarr(n_elements(lons),n_elements(lats),12)  ; annual emissions flux file
tsoutfield = fltarr(n_elements(lons),n_elements(lats),totalyears*12)  ; timeseries emiss flux

time=0
counter=0

for yr=0,totalyears-1 do begin
  ...
  for mth=0,11 do begin
     ...
     if (iyear lt 1980) then begin

       print, 'time pre 1980s'
       outfield[*,*,mth] = pre1980fld[*,*,mth]

     endif else if (iyear gt 2010) then begin

       print, 'time post 2010s'
       outfield[*,*,mth] = post2010fld[*,*,mth]

     endif else if (iyear ge 1980 && iyear le 2010) then begin

       time = time + 1

       print, 'time index = ',time
       outfield[*,*,mth] =  biogflux[*,*,time-1]

     endif

     tsoutfield[*,*,counter-1] = outfield[*,*,mth]
...
ncdf_attput,ncid,fieldvar_id,'long_name','Biogenic surface emissions of lumped non-acetone ketones, expressed as acetone'
ncdf_attput,ncid,fieldvar_id,'molecular_weight',58.08,/float
ncdf_attput,ncid,fieldvar_id,'molecular_weight_units','g mol-1'

ncdf_attput,ncid,/global,'history','MEGAN-MACC_biogenic_other_ketones_preprocess.pro'
ncdf_attput,ncid,/global,'source','MEGAN-MACC Biogenic emission inventory - Data between year 1980 and 2010 from MEGAN-MACC distributed by ECCAD'
...
ncdf_varput,ncid,fieldvar_id,tsoutfield
