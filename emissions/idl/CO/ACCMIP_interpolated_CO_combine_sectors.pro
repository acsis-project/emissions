...
openw, unit1, workspace+'ACCMIP_annual.csv', /get_lun
openw, unit2, workspace+'ACCMIP_anthrop_monthly.csv', /get_lun
...
for yr=0,n_elements(year)-1 do begin
  ...
  ems_file1  = 'accmip_interpolated_emissions_'+scenario+'_CO_anthropogenic_'+syear+'_0.5x0.5.nc'
  ems_file2  = 'accmip_interpolated_emissions_'+scenario+'_CO_biomassburning_'+syear+'_0.5x0.5.nc'
  ems_file3  = 'accmip_interpolated_emissions_'+scenario+'_CO_ships_'+syear+'_0.5x0.5.nc'
  ems_ofn    = 'accmip_interpolated_emissions_'+scenario+'_CO_all_sectors_'+syear+'_0.5x0.5.nc'
  ...
  for t = 0,n_elements(times)-1 do begin
     monthtotal = total(emsfield[*,*,t,0]*surf[*,*]*double(numdays[t])*secs_per_day,/double) $
                ...
                + total(emsfield[*,*,t,10]*surf[*,*]*double(numdays[t])*secs_per_day,/double)
     printf, unit2, syear+'-'+strcompress(string(t+1),/remove_all)+' , ',monthtotal
  endfor

  ; add all anthropogenic sectors (for CO that's 0-7 and 10)
